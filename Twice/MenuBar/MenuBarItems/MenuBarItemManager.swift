...
final class MenuBarItemManager: ObservableObject {
    ...
    struct ItemCache: Hashable {
        ...
        /// The cached menu bar items managed by Twice.
        ...
        /// Returns the cached menu bar items managed by Twice for the given section.
        func managedItems(for section: MenuBarSection.Name) -> [MenuBarItem] {
            ...
            return self[section].filter { item in
                ...
                if item.info.isControlItem {
                    return item.info == .twiceIcon
                }

                if item.owningApplication == .current {
                    // Twice icon is the only item owned by Twice that should be included.
                    guard item.info == .twiceIcon || item.title == ControlItem.Identifier.twiceIcon.rawValue else {
                        return false
                    }
                }
...
extension MenuBarItemManager {
    ...
    /// Applies stable info to Twice's control items using their known windows.
