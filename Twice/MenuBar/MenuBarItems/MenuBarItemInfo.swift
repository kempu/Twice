...
struct MenuBarItemInfo: Hashable, CustomStringConvertible {
    ...
    /// A Boolean value that indicates whether the item is one of Twice's
    /// control items.
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        if #available(macOS 26.0, *) {
            ...
            self.init(namespace: .twice, title: identifier.rawValue)
        }
    }
}
...
extension MenuBarItemInfo {
    ...
    /// An array of items that represent Twice's control items.
    static let controlItems = [twiceIcon, hiddenControlItem, alwaysHiddenControlItem]

    /// Information for an item that represents the Twice icon, a.k.a. the
    /// control item for the visible section.
    static let twiceIcon = MenuBarItemInfo(controlItem: .twiceIcon)
...
extension MenuBarItemInfo.Namespace {
    /// The namespace for menu bar items owned by Twice.
    static let twice = Self(Constants.bundleIdentifier)
