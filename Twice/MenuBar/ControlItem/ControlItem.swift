...
final class ControlItem {
    ...
    private func configureCancellables() {
        ...
        if let appState {
            appState.settingsManager.generalSettingsManager.$showTwiceIcon
                .receive(on: DispatchQueue.main)
                .sink { [weak self] showTwiceIcon in
                    guard
                        let self,
                        !isSectionDivider
                    else {
            ...
            appState.settingsManager.generalSettingsManager.$twiceIcon
                .receive(on: DispatchQueue.main)
                .sink { [weak self] _ in
                    guard let self else {
                        return
            ...
            appState.settingsManager.generalSettingsManager.$customTwiceIconIsTemplate
                .receive(on: DispatchQueue.main)
                .sink { [weak self] _ in
                    guard let self else {
                        return
    ...
    private func updateStatusItem(with state: HidingState) {
        ...
        case .visible:
            ...
            let icon = appState.settingsManager.generalSettingsManager.twiceIcon
    ...
    private func createMenu(with appState: AppState) -> NSMenu {
        ...
        let menu = NSMenu(title: "Twice")

        let settingsItem = NSMenuItem(
            title: "Twice Settings…",
