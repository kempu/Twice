...
final class TwiceBarPanel: NSPanel {
    private weak var appState: AppState?

    private(set) var currentSection: MenuBarSection.Name?

    private lazy var colorManager = TwiceBarColorManager(twiceBarPanel: self)
            ...
            window.publisher(for: \.frame)
                ...
                .sink { [weak self, weak window] _ in
                    guard
                        let self,
                        let appState,
                        // Only continue if the menu bar is automatically hidden, as Twice
    ...
    private func updateOrigin(for screen: NSScreen) {
        ...
        func getOrigin(for twiceBarLocation: TwiceBarLocation) -> CGPoint {
        ...
        setFrameOrigin(getOrigin(for: appState.settingsManager.generalSettingsManager.twiceBarLocation))
    }
    ...
    func show(section: MenuBarSection.Name, on screen: NSScreen) async {
        ...
        appState.navigationState.isTwiceBarPresented = true
        ...
        contentView = TwiceBarHostingView(appState: appState, colorManager: colorManager, screen: screen, section: section) { [weak self] in
            self?.close()
        }
    ...
    override func close() {
        ...
        contentView = nil
        currentSection = nil
        appState?.navigationState.isTwiceBarPresented = false
    }
}

// MARK: - TwiceBarHostingView

private final class TwiceBarHostingView: NSHostingView<AnyView> {
    ...
    init(
        appState: AppState,
        colorManager: TwiceBarColorManager,
        screen: NSScreen,
        section: MenuBarSection.Name,
        closePanel: @escaping () -> Void
    ) {
        super.init(
            rootView: TwiceBarContentView(screen: screen, section: section, closePanel: closePanel)
...
// MARK: - TwiceBarContentView

private struct TwiceBarContentView: View {
    @EnvironmentObject var appState: AppState
    @EnvironmentObject var colorManager: TwiceBarColorManager
    ...
    private var content: some View {
        if !ScreenCapture.cachedCheckPermissions() {
            HStack {
                Text("The Twice Bar requires screen recording permissions.")
            ...
            Text("Twice cannot display menu bar items for automatically hidden menu bars")
                .padding(.horizontal, 10)
            ...
            ScrollView(.horizontal) {
                HStack(spacing: 0) {
                    ForEach(items, id: \.windowID) { item in
                        TwiceBarItemView(item: item, closePanel: closePanel)
                    }
                }
            }
...
// MARK: - TwiceBarItemView

private struct TwiceBarItemView: View {
    @EnvironmentObject var imageCache: MenuBarItemImageCache
    @EnvironmentObject var itemManager: MenuBarItemManager
    ...
    var body: some View {
        if let image {
            Image(nsImage: image)
                ...
                .overlay {
                    TwiceBarItemClickView(item: item, leftClickAction: leftClickAction, rightClickAction: rightClickAction)
                }
...
// MARK: - TwiceBarItemClickView

private struct TwiceBarItemClickView: NSViewRepresentable {
